...
```
Ticket ID:        T-002
Title:            反思闭环自动化(append_reflection.py + run_skill.py hook)
Owner:            @Copilot(实施)· @Architect(规格 / review)
Created:          2026-04-26T08:00Z
Linked Goal:      G-001
Linked Decisions: D-002, D-003, D-004, D-005

Goal (1 sentence):
    让 agent 每次完成一个 skill 运行后,能自动生成一份反思骨架(R-NNN.md),
    把"写反思"从纯人工步骤变成系统自动触发的最小闭环。

Acceptance Criteria:
- [ ] 新增 `scripts/append_reflection.py`,CLI:
...
- [ ] 新增 `scripts/test_append_reflection.py`,python -m unittest 可跑:
        ...
        总测试数 ≥ 10
- [ ] `reflections/R-002.md` 作为 T-002 的关闭凭证(PR merge 后落档)
- [ ] PR 描述列出:做了什么 / 没做什么 / 为什么

Files in Scope:
    可改:
        scripts/append_reflection.py          (新增)
        scripts/run_skill.py                  (修改:添加 --reflect flag,不改其他逻辑)
        scripts/test_append_reflection.py     (新增,自测)
        AGENT-COLLAB.md §8                    (追加 REPORT / QUESTION 消息)
        goals/G-001.md `Last advanced`        (推进时更新)
        reflections/R-002.md                  (新增,关闭时落)
    ...
    禁改(未经 @Architect ACK 不得动):
        ...
        scripts/goal_stack.py · scripts/route.py · scripts/append_audit.py
        所有现有 `.agents/skills/<id>/skill.md`
        policies/permissions.md(本 ticket 不新增 workflow,无需改)

Constraints:
    1. 不引入新 pip 依赖(stdlib only;PyYAML 可选降级,同 T-001)。
    2. `append_reflection.py` 纯函数式风格,与 goal_stack.py 保持一致。
    3. 不调用 LLM。本 ticket 是机械工具层。
    4. `run_skill.py` 改动必须向后兼容:现有调用方式不变,`--reflect` 是纯增量。
    5. `append_reflection.py` **只创建骨架**,不自动填写四问答案。答案由人或 @Copilot 手写。
...
1. `append_reflection.py` front-matter 字段是否与 `reflections/_template.md` 一一对应。
2. 幂等逻辑是否真实:同一 ticket 第二次调用不覆盖已有文件。
3. `run_skill.py` 无 `--reflect` 时行为是否完全不变(逐行 diff)。
4. 测试覆盖三条路径(创建 / 自增 / 幂等)。
5. 是否在 Files in Scope 内(无越界)。
6. 是否落了 `reflections/R-002.md`。
...
- **Active Ticket:** T-002 反思闭环自动化 — SPEC 已发,@Copilot 实施中
