...
|---|---|---|---|
| T-006 | 主动巡视触发层(`proactive-watch.yml` + `proactive-scan` skill + 权限登记) | G-002 | T-005 完成 |
...
Created:          2026-04-29T10:30Z
Linked Goal:      G-002
Linked Decisions: D-006, D-007

Goal (1 sentence):
    实现可独立运行的仓库健康扫描引擎：读取可配置的巡视清单，
    执行 4 类检查，输出结构化 Markdown 报告，让后续 workflow 能
    据此决定是否开 issue 提醒人类。

Acceptance Criteria:
- [ ] 新增 `memory/watchlist.yml`，定义 4 个默认检查项：
        stale-goals:
          enabled: true
          threshold_days: 7
      ...
      每项必须有 id / enabled / description；threshold 字段按检查类型各异。
- [ ] 新增 `scripts/scan_repo.py`，对外 CLI：
        python scripts/scan_repo.py [--watchlist PATH] [--repo-root PATH]
      行为：
        * 读取 watchlist.yml（默认 memory/watchlist.yml）
        * 对每个 enabled=true 的检查项执行对应逻辑
        * 无发现：stdout 空 / 只有 "All checks passed."；exit 0
        * 有发现：stdout 输出 Markdown 报告（每项发现一个 ## 段落）；exit 1
        * 某项检查文件缺失时静默跳过，不 crash（同 assemble_context.py 惯例）
        * 支持 --help
      4 类检查实现细节：
        stale-goals    — 扫描 goals/G-NNN.md，解析 front-matter status + Last advanced 最新日期
        missing-reflections — 数 §9 ticket 行数（grep "^| T-" AGENT-COLLAB.md）vs ls reflections/R-[0-9]*.md
        open-questions — 解析 AGENT-COLLAB.md §5 表格，找 status=open 且日期字段非"—"的行
        audit-overrun  — 读今日 memory/audit/<date>.md，累加 Tokens 列，与 budget 比较
- [ ] 新增 `scripts/test_scan_repo.py`，python -m unittest 可跑，≥ 12 unittest：
        至少覆盖：
        - watchlist.yml 读取正确（enabled 过滤）
        - stale-goals 检测：in-progress + 超期 → 发现；done → 跳过
        - missing-reflections：ticket 数 > R 数 → 发现；相等 → 通过
        - open-questions：有 open Q → 发现；无 → 通过
        - audit-overrun：超阈值 → 发现；未超 → 通过
        - 某层文件缺失时静默跳过不 crash
        - exit 0 / exit 1 行为正确
- [ ] `reflections/R-006.md` 作为 T-005 关闭凭证

Files in Scope:
    可改：
        memory/watchlist.yml              (新增)
        scripts/scan_repo.py              (新增)
        scripts/test_scan_repo.py         (新增)
        AGENT-COLLAB.md §8               (REPORT)
        goals/G-002.md Last advanced      (推进)
        reflections/R-006.md             (新增，关闭时落)

    禁改（未经 @Architect ACK 不得动）：
        AGENTS.md · MEMORY.md
        docs/agent-*.md · AGENT-COLLAB.md §0–§7 §9–§12
        goals/README.md · goals/_template.md
        reflections/README.md · reflections/_template.md
        scripts/goal_stack.py · scripts/assemble_context.py
        scripts/append_reflection.py · scripts/run_skill.py
        .github/workflows/* · .agents/skills/*
        policies/*

Constraints:
    1. stdlib only，无新 pip 依赖。
    2. 纯函数式风格，与 assemble_context.py / goal_stack.py 保持一致。
    3. 不调用 LLM。scan_repo.py 是纯机械扫描工具。
    4. 每类检查文件缺失时静默跳过，不 crash。
    5. exit 0 = 无发现；exit 1 = 有发现。workflow 依赖此约定，不得更改。
    6. watchlist.yml 用标准 YAML（PyYAML 可选降级同 goal_stack.py 惯例）。

Linked PR / Issue: <@Copilot 在 REPORT 时填>
```

**审查路径：**
1. `watchlist.yml` 包含 4 个 check，schema 字段完整。
2. `scan_repo.py` CLI 支持 `--watchlist` / `--repo-root` / `--help`。
3. 4 类检查逻辑各自正确，文件缺失时静默跳过。
4. exit 0 / exit 1 语义正确。
5. ≥ 12 unittest，7 个指定路径全覆盖。
6. Files in Scope 无越界。
7. `reflections/R-006.md` 落档。
...
- **当前 Phase:** 3（主动巡视）— @Owner D-007 豁免入口条件，2026-04-29 正式开启
- **当前 Goal:** `goals/G-002.md` (in-progress, P0)
- **已完成:** Phase 2 ✅ (G-001 done · T-001–T-004 · R-001–R-005 · R-PHASE-2)
- **Active Ticket:** T-005 主动巡视扫描引擎 — SPEC 已发，@Copilot 实施中
- **Backlog:** T-006（触发层，等 T-005 完成）
