...
|---|---|---|---|
| T-004 | Skill 协议升级(`cognitive_mode` + `collab_tier` 字段 + validate-skills.yml 强制校验) | G-001 | T-003 完成 |
...
```
Ticket ID:        T-003
Title:            Working Set 装配(assemble_context.py + run_skill.py 替换 memory_excerpt)
Owner:            @Copilot(实施)· @Architect(规格 / review)
Created:          2026-04-26T09:00Z
Linked Goal:      G-001
Linked Decisions: D-002, D-003, D-004, D-005

Goal (1 sentence):
    把 run_skill.py 中粗暴的 memory_excerpt() 替换为一个独立的 assemble_context.py
    模块,按 §7 Phase 2 朴素方案动态装配 ContextPack,让 LLM 只收到与本次任务相关的上下文。

Acceptance Criteria:
- [ ] 新增 `scripts/assemble_context.py`,对外暴露一个公开函数:
        assemble(event: dict, repo_root: pathlib.Path = None) -> str
      内部按以下顺序装配 ContextPack(总字符数 ≤ 6000,超则按优先级截断):
        1. Standing Context:从 MEMORY.md 提取 "## Standing Context" 段(≤1500 chars)
        2. Active Goal:从 goals/ 找所有 status=in-progress 的 G-NNN.md,
           取其 title + acceptance criteria + last advanced(≤1500 chars)
        3. Recent Audit:从 memory/audit/<UTC date>.md 读最近 7 天的行,
           只保留最新 20 行(≤1500 chars)
        4. Event Hint:从 event 提取 PR/issue 涉及的文件路径列表(≤500 chars)
      返回格式为带 section header 的 Markdown 字符串,方便模型识别边界。
      无法读取某层时静默跳过(不抛异常),回退到 "(not available)"。
- [ ] 修改 `scripts/run_skill.py`:
        * 在文件顶部 import `assemble_context`(相对 import 或 sys.path 插入)
        * 把 `memory_excerpt()` 的调用点替换为 `assemble_context.assemble(event, REPO_ROOT)`
        * 若 import 失败(模块不存在),fallback 到原 `memory_excerpt()` 并打印 WARNING
        * 禁止删除 `memory_excerpt()` 函数本身(fallback 需要它)
- [ ] 新增 `scripts/test_assemble_context.py`,python -m unittest 可跑,总测试数 ≥ 10:
        至少覆盖:
        - Standing Context 提取正确
        - Active Goal 提取正确(含 in-progress 过滤)
        - Recent Audit 截取最新 20 行
        - Event Hint 从 PR event 提取文件列表
        - 某层文件缺失时静默跳过
        - 总长度超限时截断(≤ 6000 chars)
        - run_skill.py 在有 assemble_context 时调用它而非 memory_excerpt
- [ ] `reflections/R-003.md` 作为 T-003 的关闭凭证(PR merge 后落档)
- [ ] PR 描述列出:做了什么 / 没做什么 / 为什么

Files in Scope:
    可改:
        scripts/assemble_context.py           (新增)
        scripts/run_skill.py                  (修改:替换 memory_excerpt 调用点 + fallback)
        scripts/test_assemble_context.py      (新增,自测)
        AGENT-COLLAB.md §8                    (追加 REPORT / QUESTION 消息)
        goals/G-001.md `Last advanced`        (推进时更新)
        reflections/R-003.md                  (新增,关闭时落)
    ...
    禁改(未经 @Architect ACK 不得动):
        ...
        scripts/goal_stack.py · scripts/append_reflection.py
        scripts/route.py · scripts/append_audit.py
        所有现有 `.agents/skills/<id>/skill.md`
        policies/permissions.md(本 ticket 不新增 workflow,无需改)

Constraints:
    1. 不引入新 pip 依赖(stdlib only)。
    2. `assemble_context.py` 纯函数式风格,与 goal_stack.py / append_reflection.py 保持一致。
    3. 不调用 LLM。本 ticket 是工具层装配,不是 cognitive 层。
    4. assemble() 必须在任何记忆层文件缺失时都能正常返回字符串(不 crash)。
    5. run_skill.py 改动必须向后兼容:若 assemble_context 不可用,行为与修改前完全一致。
    6. 单 PR 范围:**仅本 ticket**。越界改动记到 "Out-of-scope observations"。

Linked PR / Issue: <@Copilot 在 REPORT 时填>
```

**审查路径:** @Copilot 提交 PR 后,@Architect 将按以下顺序 REVIEW:
1. `assemble_context.assemble()` 签名是否匹配规格:`event: dict, repo_root: pathlib.Path = None`。
2. 四层装配顺序(Standing Context → Active Goal → Recent Audit → Event Hint)是否正确。
3. 超限截断逻辑:总 ≤ 6000 chars,按层优先级截断而非简单 truncate。
4. 静默跳过:每层文件缺失时不抛异常。
5. `run_skill.py` fallback:import 失败时退回 `memory_excerpt()` 且不影响功能。
6. 测试覆盖 ≥10 条,七个指定路径全覆盖。
7. 是否在 Files in Scope 内(无越界)。
8. 是否落了 `reflections/R-003.md`。
...
- **已完成:** T-001 ✅ Goal Stack CLI · T-002 ✅ 反思闭环自动化(R-001 / R-002 已落档)
- **Active Ticket:** T-003 Working Set 装配 — SPEC 已发,等待 @Copilot ACK
- **预计本 Phase 剩余 ticket 数:** 2 (T-003 → T-004)
