...
>
> 适用范围:Phase 2 起 · 维护者:@Architect · 日期:2026-04-25
...
Phase 1 的 `model_tier: simple|complex` 只区分了"贵不贵",没区分"思维方式不同"。Phase 2 起,每个 skill 应同时声明 `cognitive_mode: reflexive|deliberative|reflective`。
...
| 记忆类型 | 内容 | 谁写 | 半衰期 |
|---|---|---|---|
| **身份 (Identity)** | "我是谁、我的工作方式" | 极少改,人类把关 | 永久 |
| **语义 (Semantic)** | 关于这个仓库的领域知识、约定、决策依据 | agent 提议 + 人类批准 | 长期 |
| **程序 (Procedural)** | 我会哪些技能、每个技能怎么做 | agent 自己积累 + 人类 review | 长期 |
| **情景 (Episodic)** | 我什么时候做过什么、结果如何 | agent 自动写,append-only | 中期(90 天后归档) |
| **工作 (Working)** | 当前任务的上下文 | 临时,任务结束即清理 | 短期(单次激活) |
| **目标栈 (Goal Stack)** ⭐ | 当前所有进行中的目标、状态、阻塞项 | agent 持续维护 | 直到目标完成 |
...
每个进行中的目标是一个独立文件(实际位置:`goals/G-NNN.md`),内容包括:

- 目标的一句话陈述
- 接受标准(怎么算"完成")
- 当前状态(在哪一步、谁是阻塞方)
- 子任务清单及进度
- 关联的 issue / PR / commit / decision
- 上次推进的时间和当时学到的东西
...
反思笔记落在 `reflections/R-NNN.md`。**这是 agent 真正"学习"的入口**——不需要任何 fine-tuning,纯靠文件就能积累。
...
- **Propose 档** —— 产出 PR / draft,人类 merge 决定。例:`/memory-write`、文档改动。
- **Execute 档** —— 直接 commit 到 main。**仅限低风险、高确定性、有 schema 校验** 的事。例:格式化、修 typo、归档过期 audit 日志。

权限矩阵应改为 **"skill × 协作档位"** 的二维结构。

---

## 11. 完整运行循环

每次 agent 被激活,都走以下 6 步:
...
| **2** | 拥有"持续意图" | Goal Stack;反思闭环;Working Set 装配 |
| **3** | 主动巡视 | 定时 workflow + 巡视清单 |
| **4** | 学习与演化 | 反思影响 Working Set;skill 自动演化候选;embedding 检索 |
| **5** | 多 agent 协作 | 不同 agent 持有不同 Goal Stack,通过 issue/PR 协调 |
...
**实施顺序建议:** **先实现 Goal Stack 与反思闭环(§6 + §9)** —— 这两个是最少改动、最大杠杆的入口。
